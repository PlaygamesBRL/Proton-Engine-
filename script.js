...
function init() {
    ...
    // Carregar modelo da câmera
    var loader = new THREE.OBJLoader();
    loader.load('engine/Camera/camera.obj', function (object) {
        object.traverse(function (child) {
            if (child instanceof THREE.Mesh) {
                // Criar material com textura
                var textureLoader = new THREE.TextureLoader();
                var texture = textureLoader.load('engine/Camera/camera.png');
                var material = new THREE.MeshBasicMaterial({ map: texture });
                child.material = material;
            }
        });
        // Adicionar rotação ao objeto da câmera
        object.rotation.y = Math.PI; // Por exemplo, rotaciona 180 graus em torno do eixo y
        scene.add(object);
    });

    // Adicionar helper da câmera
...
importModelButton.onclick = toggleImportPanel;
    ...
    optionsPanel.innerHTML = '<h3>Model Options</h3>' +
        '<button onclick="loadModel(\'engine/models/sphere.obj\', 0.5, 1.0)">Sphere</button>' +
        '<button onclick="loadModel(\'engine/models/cube.obj\', 0.5, 1.0)">Cube</button>' +
        '<button onclick="loadModel(\'engine/models/torus.obj\', 0.5, 1.0)">Torus</button>' +
        '<button onclick="loadModel(\'engine/models/capsule.obj\', 0.5, 1.0)">Capsule</button>';
...
propertiesPanel.innerHTML = '<label for="textureInput">Importar Textura:</label>' +
    ...
    '<select id="materialDropdown" onchange="applyMaterial(this.value)">' +
    ...
    '</select>' +
    '<label for="modelDropdown">Modelo:</label>' +
    '<select id="modelDropdown" onchange="applyModel(this.value)">' +
...
// Adiciona o painel de configuração ao documento
...
// Aplica uma textura no modelo atualmente selecionado
...
// Define a cor albedo no modelo atualmente selecionado
function setAlbedo() {
...
// Aplica um mapa de normal no modelo atualmente selecionado
...
// Define a rugosidade no modelo atualmente selecionado
...
// Define a metalicidade no modelo atualmente selecionado
        ...
        switch (materialType) {
            ...
            case 'transparent':
                material = new THREE.MeshStandardMaterial({
                    ...
                    roughness: 0.5, // Adicione quaisquer outras propriedades desejadas para o material transparente
                    metalness: 0.5
                });
                break;
            case 'lambert':
...
// Aplica o modelo selecionado ao modelo atualmente selecionado
function applyModel(modelType) {
    var geometry;
    ...
    switch (modelType) {
        ...
        case 'torus':
            ...
            break;
        default:
            console.error('Modelo não reconhecido');
            return;
    }
    ...
    // Remove o modelo atual, se existir
    if (currentModel) {
        scene.remove(currentModel);
    }
...
function importFBXModel() {
    ...
    if (file) {
        ...
        reader.readAsArrayBuffer(file); // Usar readAsArrayBuffer() para ler o arquivo .fbx
    }
}
...
function createConfigPanel() {
    ...
    configPanel.className = 'panel-config';
    configPanel.innerHTML = '<h3>Configuração</h3>' +
        '<label for="fpsLimitInput">Limitador de FPS:</label>' +
        '<input type="number" id="fpsLimitInput" min="1" max="240" step="1" value="60">' +
        '<button onclick="setFPSLimit()">Definir</button>';
    return configPanel;
}

// Função para definir o limite de FPS
function setFPSLimit() {
    var fpsLimitInput = document.getElementById('fpsLimitInput');
    var fpsLimit = parseInt(fpsLimitInput.value);
    // Defina aqui a lógica para limitar o FPS para o valor especificado
    console.log('Limitar FPS para: ' + fpsLimit);
}

// Função para alternar a exibição do painel de configuração
function toggleConfigPanel() {
    var configPanel = document.querySelector('.panel-config');
